locals {
  ...
  base_permission_scope_map = {
    for r in var.permission_scopes : r.value => {
      ...
      user_consent_display_name  = try(r.user_consent_description, r.admin_consent_description)
      user_consent_description   = try(r.user_consent_display_name, r.admin_consent_display_name)
